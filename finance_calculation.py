...
class Finance:
    ...
    def calculate_days_and_send_msg(self):
        ...
        elif date_given.lower() in ['no', 'n']:
            start_date = (input("Enter the date in YYYY-MM-DD format \n"))

            try:
                start_date = datetime.strptime(start_date, '%Y-%m-%d')
                print("successfully parsed date :", start_date)
        ...
        months_amount = [
            ((start_date + relativedelta(months=i+1)).strftime("%Y-%m-%d"), amount)
